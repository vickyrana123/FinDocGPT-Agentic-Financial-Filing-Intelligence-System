...
def answer_query(query: str, history: list[dict] | None = None) -> dict:
    ...
    if route == "general":
        return {"answer": general_response(query), "route": route, "ticker": None}

    ticker = extract_ticker(query, history=history)

    if route == "live":
        if not ticker:
            return {
                "answer": "I couldn't find a ticker in your question. Try including one, e.g. 'AAPL current price'.",
                "route": route, "ticker": None,
            }
        return {"answer": format_price_result(get_stock_price(ticker)), "route": route, "ticker": ticker}

    elif route == "filings":
        return {"answer": query_filings(query, ticker=ticker, history=history), "route": route, "ticker": ticker}

    elif route == "both":
        live_part = format_price_result(get_stock_price(ticker)) if ticker else "No ticker found for live data."
        filing_part = query_filings(query, ticker=ticker, history=history)
        return {
            "answer": f"Live data:\n{live_part}\n\nFrom filings:\n{filing_part}",
            "route": route, "ticker": ticker,
        }

    return {"answer": "Sorry, I couldn't determine how to answer that.", "route": "unknown", "ticker": None}
